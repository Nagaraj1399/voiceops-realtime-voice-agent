...
export const LiveAgentPage: React.FC<LiveAgentPageProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="px-3.5 py-2.5 bg-neutral-900/60 border border-neutral-800 rounded-xl flex flex-wrap items-center justify-between gap-2.5 text-xs">
        <div className="flex items-center gap-2 text-neutral-300">
          <Info className="w-4 h-4 text-indigo-400 shrink-0" />
          <span className="leading-snug">
            <strong>Hackathon Live Demo:</strong> VoiceOps AI turns conversational audio directly into verified tool execution.
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 text-neutral-400 font-mono text-[11px]">
          ...
          <span className="text-emerald-400 font-semibold">Barge-in Active</span>
        </div>
      </div>

      {/* MOBILE ONLY: Sticky Live Call Status Header Bar when active */}
      {isCallActive && (
        <div className="lg:hidden p-3 bg-neutral-900/90 border border-neutral-800 rounded-xl flex items-center justify-between shadow-lg animate-fadeIn">
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              <span className="text-xs font-bold text-neutral-200 uppercase font-mono block">
          ...
          <div className="flex items-center gap-2">
            {agentState === 'speaking' && (
              <button
                onClick={onInterrupt}
                className="px-2.5 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 rounded-lg text-xs font-mono font-medium flex items-center gap-1 active:scale-95"
              >
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-rose-600 hover:bg-rose-500 text-white rounded-lg text-xs font-medium flex items-center gap-1 active:scale-95"
            >
      ...
      <div className="lg:hidden flex items-center p-1 bg-neutral-900 border border-neutral-800 rounded-xl text-xs font-medium">
        <button
        ...
        >
          ...
          {messages.length > 0 && (
            <span className="px-1.5 py-0.2 rounded-full bg-neutral-700 text-[10px] font-mono text-neutral-300">
              {messages.length}
            </span>
          )}
        </button>
        ...
        >
          ...
          {toolCalls.length > 0 && (
            <span className="px-1.5 py-0.2 rounded-full bg-emerald-950 text-emerald-300 border border-emerald-800 text-[10px] font-mono">
              {toolCalls.length}
            </span>
          )}
        </button>
          ...
          <div className="h-[calc(100dvh-17rem)] min-h-[420px] flex flex-col animate-fadeIn">
            ...
            <form onSubmit={handleSend} className="mt-2.5 flex items-center gap-2">
              <input
                ...
                className="flex-1 px-3 py-2.5 text-sm sm:text-xs bg-neutral-900/90 border border-neutral-800 rounded-lg text-neutral-200 placeholder-neutral-400 focus:outline-none focus:border-indigo-500 transition-colors"
              />
              <button
                type="submit"
                aria-label="Send message"
                className="min-h-[44px] min-w-[44px] flex items-center justify-center bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors shrink-0 active:scale-95"
              >
        ...
        <div className="lg:col-span-4 h-[680px] flex flex-col">
          ...
          <form onSubmit={handleSend} className="mt-2.5 flex items-center gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 text-xs bg-neutral-900/90 border border-neutral-800 rounded-lg text-neutral-200 placeholder-neutral-400 focus:outline-none focus:border-indigo-500 transition-colors"
            />
            <button
              type="submit"
